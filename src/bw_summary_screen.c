...
static EWRAM_DATA struct PokemonSummaryScreenData
{
    ...
    s16 shadowSwitchCounter; // NEW: Dedicated counter for the shadow sprite
...
static const u8 sText_Switch[]                              = _("Tauschen");
static const u8 sText_PkmnInfo[]                            = _("Pokémon Info");
static const u8 sText_PkmnSkills[]                          = _("Status");
static const u8 sText_BattleMoves[]                         = _("Attacken");
static const u8 sText_ContestMoves[]                        = _("Contest Moves");
static const u8 sText_Info[]                                = _("Info");
static const u8 sText_ViewIVs[]                             = _("IVs");
static const u8 sText_ViewEVs[]                             = _("EVs");
static const u8 sText_ViewStats[]                           = _("Status");
static const u8 sText_ViewIVs_Graded[]                      = _("See Innate");
static const u8 sText_ViewEVs_Graded[]                      = _("See Effort");
static const u8 sText_NextLv[]                              = _("Nächstes Lv.");
static const u8 sText_RentalPkmn[]                          = _("Leih-Pokémon");
...
{
    ShowPokemonSummaryScreen_BW(SUMMARY_MODE_SELECT_MOVE, mons, monIndex, gPartiesCount[B_TRAINER_PLAYER], callback);
...
static bool8 LoadGraphics(void)
{
    ...
    case 15:
        ...
        sMonSummaryScreen->switchCounter = 0;
        sMonSummaryScreen->shadowSwitchCounter = 0; // ADD THIS LINE
        gMain.state++;
        break;
    case 16:
        ...
        // UPDATE THE IF STATEMENT SO IT WAITS FOR BOTH TO FINISH:
        if (sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_MON] != SPRITE_NONE && (!BW_SUMMARY_MON_SHADOWS || sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_SHADOW] != SPRITE_NONE))
        {
            sMonSummaryScreen->monAnimTimer = 0;
...
static void Task_ChangeSummaryMon(u8 taskId)
{
    ...
    case 7:
        ...
        data[1] = 0;
        data[2] = 0; // ADD THIS LINE
        break;
    case 8:
        sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_MON] = LoadMonGfxAndSprite(&sMonSummaryScreen->currentMon, &data[1], FALSE);

        if (BW_SUMMARY_MON_SHADOWS)
            sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_SHADOW] = LoadMonGfxAndSprite(&sMonSummaryScreen->currentMon, &data[2], TRUE);
...
static void PrintHPStats(u8 mode)
{
    ...
    if (mode == SKILL_STATE_STATS)
        PrintTextOnWindow(windowId, gStringVar4, 19, 0, 0, 0);
    else
        PrintTextOnWindow(windowId, gStringVar4, 6, 0, 0, 0);
}
...
static void PrintExpPointsNextLevel(void)
{
    ...
    PrintTextOnWindow(windowIdNextLvl, gStringVar1, 2, 4, 0, 0);
}
...
static void ShowCancelOrRenamePrompt(void)
{
    const u8 *gText_Rename = COMPOUND_STRING("Kosename");
...
static void CB2_ReturnToSummaryScreenFromNamingScreen(void)
{
    ...
    ShowPokemonSummaryScreen_BW(SUMMARY_MODE_NORMAL, gParties[B_TRAINER_PLAYER], gSpecialVar_0x8004, gPartiesCount[B_TRAINER_PLAYER], gInitialSummaryScreenCallback);
}
