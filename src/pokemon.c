...
// Verify that BoxPokemon fits in the expected 64-byte size
...
// Encryption removed - BoxPokemon data is stored in plaintext
// Checksum removed - no longer needed without encryption
...
u32 GetBoxMonData3(struct BoxPokemon *boxMon, s32 field, u8 *data)
{
    ...
    case MON_DATA_SPECIES_OR_EGG:
        ...
        if (retVal && (boxMon->isEgg || boxMon->isBadEgg))
            retVal = SPECIES_EGG;
        break;
    ...
    case MON_DATA_PP3:
    case MON_DATA_PP4:
        // PP is not stored in BoxPokemon
        break;
    case MON_DATA_HP_EV:
        retVal = boxMon->hpEV;
        break;
    ...
    case MON_DATA_POKERUS:
        retVal = boxMon->pokerusType;
        break;
    case MON_DATA_POKERUS_STRAIN:
    case MON_DATA_POKERUS_DAYS_LEFT:
        // Only pokerusType flag stored in BoxPokemon
        break;
    ...
    case MON_DATA_IS_EGG:
        retVal = boxMon->isEgg || boxMon->isBadEgg;
        break;
    ...
    case MON_DATA_CHECKSUM:
        // These are stored in struct Pokemon, not BoxPokemon
        break;
...
void SetBoxMonData(struct BoxPokemon *boxMon, s32 field, const void *dataArg)
{
    ...
    case MON_DATA_SPECIES:
    {
        ...
        boxMon->hasSpecies = (boxMon->species != 0);
        break;
    }
    case MON_DATA_HELD_ITEM:
...
void MonRestorePP(struct Pokemon *mon)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        if (GetBoxMonData(&mon->box, MON_DATA_MOVE1 + i, 0))
        {
            enum Move move = GetBoxMonData(&mon->box, MON_DATA_MOVE1 + i, 0);
            u16 bonus = GetBoxMonData(&mon->box, MON_DATA_PP_BONUSES, 0);
            mon->pp[i] = CalculatePPWithBonus(move, bonus, i);
        }
    }
}
...
{
    // PP is not stored in BoxPokemon, recalculated on unbox
    (void)boxMon;
}
