...
// The doubles pair parks in the empty leading slot of the row, one tile before
// the nickname, so the layout reads [icon][gap][name][gap][gender][gap][Lv].
#define TYPE_ICON_PLAYER_SINGLE_X_OFFSET  77
#define TYPE_ICON_PLAYER_SINGLE_Y_OFFSET (-1)
#define TYPE_ICON_PLAYER_DOUBLE_X_OFFSET (-24)
#define TYPE_ICON_OPPONENT_X_OFFSET      76
...
static void SetTypeIconXY(s32 *x, s32 *y, u32 position, bool32 useDoubleBattleCoords, enum Type types[], u32 typeNum)
{
    ...
    *y = healthbox->y + (IsOnPlayerSide(battler) ? TYPE_ICON_RESTING_Y_OFFSET : TYPE_ICON_RESTING_Y_OFFSET + 1);
