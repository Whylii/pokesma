...
static void PrintOptionLabels(void);
...
{
    [WINDOW_HEADER] = // Prints Weiter, Map, and Playtime
    {
        .bg = 0,            // which bg to print text on
        .tilemapLeft = 0,   // position from left (per 8 pixels)
        .tilemapTop = 1,    // position from top (per 8 pixels)
        .width = 30,        // width (per 8 pixels) - full screen
    ...
    [WINDOW_MIDDLE] = // Prints the name, dex number, and badges
    {
        ...
        .paletteNum = 0,           // palette index to use for text
        .baseBlock = 1 + (30 * 2), // tile start in VRAM (after HEADER)
    },

    [WINDOW_NEW_GAME] = // Prints New Game
    {
        .bg = 0,
        .tilemapLeft = 0,
        .tilemapTop = 12,
        ...
        .height = 3,
        .paletteNum = 0,
        .baseBlock = 1 + (30 * 2) + (18 * 8),
    },

    [WINDOW_OPTIONS] = // Prints Options
    {
        .bg = 0,
        .tilemapLeft = 15,
        .tilemapTop = 12,
        ...
        .height = 3,
        .paletteNum = 0,
        .baseBlock = 1 + (30 * 2) + (18 * 8) + (15 * 3),
    },

    [WINDOW_MYSTERY_GIFT] = // Prints Mystery Gift
    {
        .bg = 0,
        .tilemapLeft = 0,
        .tilemapTop = 16,
        ...
        .height = 3,
        .paletteNum = 0,
        .baseBlock = 1 + (30 * 2) + (18 * 8) + (15 * 3) + (15 * 3),
    },

    [WINDOW_MYSTERY_EVENT] = // Prints Mystery Events
    {
        .bg = 0,
        .tilemapLeft = 15,
        .tilemapTop = 16,
        ...
        .height = 3,
        .paletteNum = 0,
        .baseBlock = 1 + (30 * 2) + (18 * 8) + (15 * 3) + (15 * 3) + (15 * 3),
    },
    DUMMY_WIN_TEMPLATE
};
...
static const u32 sMainBgTilesFem[] = INCGFX_U32("graphics/ui_main_menu/main_tiles_fem.png", ".4bpp.lz");
static const u32 sMainBgTilemapFem[] = INCGFX_U32("graphics/ui_main_menu/main_tiles_fem.bin", ".lz");
static const u16 sMainBgPaletteFem[] = INCGFX_U16("graphics/ui_main_menu/main_tiles_fem.png", ".gbapal");

static const u32 sScrollBgTiles[] = INCGFX_U32("graphics/ui_main_menu/scroll_tiles.png", ".4bpp.lz");
static const u32 sScrollBgTilemap[] = INCGFX_U32("graphics/ui_main_menu/scroll_tiles.bin", ".lz");
...
static const u32 sIconBox_GfxFem[] = INCGFX_U32("graphics/ui_main_menu/icon_shadow_fem.png", ".4bpp.lz");

static const u16 sBrendanMugshot_Pal[] = INCGFX_U16("graphics/ui_main_menu/brendan_mugshot.png", ".gbapal");
static const u32 sBrendanMugshot_Gfx[] = INCGFX_U32("graphics/ui_main_menu/brendan_mugshot.png", ".4bpp.lz");
static const u16 sMayMugshot_Pal[] = INCGFX_U16("graphics/ui_main_menu/may_mugshot.png", ".gbapal");
static const u32 sMayMugshot_Gfx[] = INCGFX_U32("graphics/ui_main_menu/may_mugshot.png", ".4bpp.lz");
...
static void PrintOptionLabels(void)
{
    ...
    // New Game (row 2 left)
    AddTextPrinterParameterized4(WINDOW_NEW_GAME, FONT_NORMAL,
        GetStringCenterAlignXOffset(FONT_NORMAL, gText_MainMenuNewGame, 104), 8,
        0, 0, colors, TEXT_SKIP_DRAW, gText_MainMenuNewGame);

    // Options (row 2 right)
    AddTextPrinterParameterized4(WINDOW_OPTIONS, FONT_NORMAL,
        GetStringCenterAlignXOffset(FONT_NORMAL, gText_MainMenuOption, 104) - 8, 8,
        0, 0, colors, TEXT_SKIP_DRAW, gText_MainMenuOption);

    // Mystery Gift / Events (row 3) - only if applicable
    switch (menuType)
    {
    case HAS_MYSTERY_GIFT:
        AddTextPrinterParameterized4(WINDOW_MYSTERY_GIFT, FONT_NORMAL,
            GetStringCenterAlignXOffset(FONT_NORMAL, gText_MainMenuMysteryGift, 104) + 8, 8,
            0, 0, colors, TEXT_SKIP_DRAW, gText_MainMenuMysteryGift);
        break;
    case HAS_MYSTERY_EVENTS:
        AddTextPrinterParameterized4(WINDOW_MYSTERY_GIFT, FONT_NORMAL,
            GetStringCenterAlignXOffset(FONT_NORMAL, gText_MainMenuMysteryGift2, 104) + 8, 8,
            0, 0, colors, TEXT_SKIP_DRAW, gText_MainMenuMysteryGift2);
        AddTextPrinterParameterized4(WINDOW_MYSTERY_EVENT, FONT_NORMAL,
            GetStringCenterAlignXOffset(FONT_NORMAL, gText_MainMenuMysteryEvents, 104), 8,
            0, 0, colors, TEXT_SKIP_DRAW, gText_MainMenuMysteryEvents);
        break;
    }
...
static const u8 sText_Badges[] = _("Orden {STR_VAR_1}");
static void PrintToWindow(u8 windowId, u8 colorIdx)
{
    const u8 colors[3] = {0,  2,  3}; 
    ...
    // Print Continue label (header left)
    AddTextPrinterParameterized4(WINDOW_HEADER, FONT_NORMAL, 18, 1,
        0, 0, colors, TEXT_SKIP_DRAW, gText_MainMenuContinue);

    // Print Map Name In Header (centered after "Weiter")
    withoutPrefixPtr = &(mapDisplayHeader[3]);
    GetMapName(withoutPrefixPtr, GetCurrentRegionMapSectionId(), 0);
    mapDisplayHeader[0] = EXT_CTRL_CODE_BEGIN;
    mapDisplayHeader[1] = EXT_CTRL_CODE_HIGHLIGHT;
    mapDisplayHeader[2] = TEXT_COLOR_TRANSPARENT;
    AddTextPrinterParameterized4(WINDOW_HEADER, FONT_NARROW, GetStringCenterAlignXOffset(FONT_NARROW, withoutPrefixPtr, 176) + 24, 1, 0, 0, colors, 0xFF, mapDisplayHeader);

    // Print Playtime In Header (right-aligned)
    ...
    *playTimePtr = 0xF0;
    ...
    AddTextPrinterParameterized4(WINDOW_HEADER, FONT_NORMAL, GetStringRightAlignXOffset(FONT_NORMAL, gStringVar4, 240) - 24, 1, 0, 0, colors, TEXT_SKIP_DRAW, gStringVar4);
    ...
    if (FlagGet(FLAG_SYS_POKEDEX_GET) == TRUE)
    {
        ...
        AddTextPrinterParameterized4(WINDOW_MIDDLE, FONT_NORMAL, 8 + 8, 24, 0, 0, colors, TEXT_SKIP_DRAW, gStringVar4);
    }
    ...
    AddTextPrinterParameterized4(WINDOW_MIDDLE, FONT_NORMAL, 16, 38, 0, 0, colors, TEXT_SKIP_DRAW, gStringVar4);
